...
class _StarFieldState extends State<StarField> {
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
            ...
            colors: [Color(0xFF010105), Color(0xFF060a14)]),
      ),
  ...
  Star _randomizeStar(Star star, bool randomZ) {
    ...
    if (rand.nextDouble() < .1) {
      //STAR Color 1
    Color(0xffD4A1FF); //purple
      star.size = 2 + rand.nextDouble() * 2;
    } else if (rand.nextDouble() < .2) {
      //STAR Color 2
      Color(0XFF37F8FF).withOpacity(0.8); //blue
      star.size = 2 + rand.nextDouble() * 2;
    } else {
  ...
  void _loadGlowImage() async {
    final ByteData data =
        await rootBundle.load('${MyApp.bundle}/assets/images/glow.png');
